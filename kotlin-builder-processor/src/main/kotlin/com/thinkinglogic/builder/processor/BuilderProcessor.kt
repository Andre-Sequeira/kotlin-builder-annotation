...
class BuilderProcessor : AbstractProcessor() {
	...
	private fun writeBuilder(classToBuild: TypeElement, fields: List<VariableElement>, sourceRoot: File, annotatedElement: Element = classToBuild) {
		...
		val builderName = when (classToBuild.nestingKind) {
			NestingKind.MEMBER -> getMemberBuilderName(classToBuild)
			else -> classToBuild.simpleName
		}

		val builderClassName = "${builderName}Builder"
	...
	private fun getMemberBuilderName(classToBuild: TypeElement): String = when (classToBuild.nestingKind) {
		NestingKind.MEMBER -> "${getMemberBuilderName(classToBuild.enclosingElement as TypeElement)}${classToBuild.simpleName}"
		else -> classToBuild.simpleName.toString()
	}
